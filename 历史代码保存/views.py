...
class QuestionPaser:
    ...
    def sql_transfer(self, question_type, entities):
        ...
        # 查询疾病有哪些症状
            ...
            sql = ["MATCH (m:disease)-[r:function]->(n) where m.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
        ...
        # 查询疾病的并发症
            ...
            sql = ["MATCH (m:drug)-[r]->(n:pharmacological_study) where m.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
        # 查询疾病的忌口
        elif question_type == 'drug_dosage':
            sql = ["MATCH (m:drug)-[r]->(n:dosage) where m.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
        elif question_type == 'drug_indication':
            sql = ["MATCH (m:drug)-[r]->(n:indication) where m.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
        elif question_type == 'disease_not_food':
        ...
        # 查询疾病常用药品－药品别名记得扩充
            ...
            sql = ["MATCH (m:disease)-[r]-(n:drug) where m.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]


        # 已知药品查询能够治疗的疾病
        elif question_type == 'drug_disease':
            sql = ["MATCH (m:drug)-[r]-(n:disease) where m.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
...
class ChatBotGraph:
    ...
    def chat_main(self, sent):
        answer = '您好，我是百奥知智能助理，希望可以帮到您。如果没答上来，可联系Rabin！'
